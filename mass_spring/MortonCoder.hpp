...
inline uint32_t spread_bits(uint32_t x) 
{
    ...
    for(unsigned int i = 1; i <= m_bits; i++)
    {
        uint32_t first = 1 << i;
        uint32_t final = x & first;
        uint32_t shift = final << 2*i;
        d = d | shift;
    }
    d &= ~(1 << 30); 
    return d;
...
{
    uint32_t d = (x % 2);
    unsigned int t_bits = 10;
    for(unsigned int i = 1; i <= t_bits; i++)
    {
        uint32_t first = 1 << 3*i;
        uint32_t final = x & first;
        uint32_t shift = final >> 2*i;
        d = d | shift;
    }
